...
The apps are built with [fastlane](./fastlane/Fastfile) lanes which uses Gradle underneath. Prepare it once with `mise install && bundle install`, then run one of these with `bundle exec <lane>`:
